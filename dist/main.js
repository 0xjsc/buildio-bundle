...
window.addEventListener('resize', _libs_utils_js__WEBPACK_IMPORTED_MODULE_2__["default"].checkTrusted(resize)), resize(), setUsingTouch(!1), window.setUsingTouch = setUsingTouch, window.addEventListener('touchmove', _libs_utils_js__WEBPACK_IMPORTED_MODULE_2__["default"].checkTrusted(function (ev) {
...
}), !1), window.addEventListener('touchstart', _libs_utils_js__WEBPACK_IMPORTED_MODULE_2__["default"].checkTrusted(function (ev) {
  if (ev.toElement.id !== "gameCanvas" &&
    ev.toElement.id !== "touch-controls-fullscreen") return;
  if (!inGame)
...
}), false), window.addEventListener('touchend', _libs_utils_js__WEBPACK_IMPORTED_MODULE_2__["default"].checkTrusted(touchEnd), !1), window.addEventListener('touchcancel', _libs_utils_js__WEBPACK_IMPORTED_MODULE_2__["default"].checkTrusted(touchEnd), !1), window.addEventListener('touchleave', _libs_utils_js__WEBPACK_IMPORTED_MODULE_2__["default"].checkTrusted(touchEnd), !1), window.addEventListener('mousemove', function (e) {
...
function killObject(sid) {
  ...
  players.find(e => e.sid != player.sid && Math.hypot(player.x - e.x, player.y - e.y) < _js_data_items_js__WEBPACK_IMPORTED_MODULE_6__["default"].weapons[player.weaponIndex].range + _config_js__WEBPACK_IMPORTED_MODULE_4__["default"].playerScale) && autoplace(object, true);
}
...
function autoplace(enemy, replace = false) {
  ...
  placers = [...toAngles(preplacableObjects), ...freeAngles].map((angle, i, array) => {
    ...
    place(player.items[((preplace || replace) && Math.abs(enemyDir - angle) < Math.PI / 2) ? 2 : (((Math.abs(angle - getMoveDir()) <= Math.PI / 2) && distance < _js_data_items_js__WEBPACK_IMPORTED_MODULE_6__["default"].weapons[player.weaponIndex].range + _config_js__WEBPACK_IMPORTED_MODULE_4__["default"].playerScale) ? 2 : 4)], angle);
...
function autobreak(trap) {
  ...
  const buildDamage = _js_data_items_js__WEBPACK_IMPORTED_MODULE_6__["default"].weapons[waka].dmg * _config_js__WEBPACK_IMPORTED_MODULE_4__["default"].fetchVariant(player).val * 
        _js_data_items_js__WEBPACK_IMPORTED_MODULE_6__["default"].weapons[waka].sDmg || 1;
...
function normalInsta() {
  const enemies = players.filter(e => e?.alive && e?.health > 0 && Math.hypot(player.x - e?.x, player.y - e?.y) < _js_data_items_js__WEBPACK_IMPORTED_MODULE_6__["default"].weapons[player.weaponIndex].range + _config_js__WEBPACK_IMPORTED_MODULE_4__["default"].playerScale && player.sid != e.sid && !alliancePlayers.includes(e.sid))
...
function reverseInsta() {
  const enemy = players.find(e => e?.alive && e?.health > 0 && Math.hypot(player.x - e?.x, player.y - e?.y) < _js_data_items_js__WEBPACK_IMPORTED_MODULE_6__["default"].weapons[player.weaponIndex].range + _config_js__WEBPACK_IMPORTED_MODULE_4__["default"].playerScale && player.sid != e.sid && !alliancePlayers.includes(e.sid));
  window.sidFocus = enemy?.sid || 69420;
